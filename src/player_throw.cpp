...
static void inventoryDropOrThrowItem(Coord_t coord, Inventory_t *item) {
    ...
    if (flag) {
        ...
        (void) snprintf(msg, 160, "The %s disappears.", description);
        printMessage(msg);
    }
}
        ...
        if (tile.feature_id <= MAX_OPEN_SPACE && !flag) {
            if (tile.creature_id > 1) {
                ...
                if (playerTestBeingHit(tbth, (int) py.misc.level, tpth, (int) creatures_list[m_ptr.creature_id].ac, PlayerClassLevelAdj::BTHB)) {
                    ...
                    if (!m_ptr.lit) {
                        (void) snprintf(msg, 160, "You hear a cry as the %s finds a mark.", description);
                        visible = false;
                    } else {
                        (void) snprintf(msg, 160, "The %s hits the %s.", description, creatures_list[damage].name);
                        visible = true;
                    }
                    printMessage(msg);
                    ...
                    if (damage >= 0) {
                        if (!visible) {
                            ...
                            (void) snprintf(msg, 160, "You have killed the %s.", creatures_list[damage].name);
                            printMessage(msg);
                        }
