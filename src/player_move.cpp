...
static void carry(Coord_t coord, bool pickup) {
    ...
    if (tile_flags == TV_GOLD) {
        ...
        itemDescription(description, item, true);
        (void) snprintf(msg, 160, "You have found %d gold pieces worth of %s", item.cost, description);
    ...
    if (inventoryCanCarryItemCount(item)) {
        ...
        if (pickup) {
            ...
            (void) snprintf(msg, 160, "You have %s (%c)", description, locn + 'a');
            printMessage(msg);
            (void) dungeonDeleteObject(coord);
        }
    } else {
        itemDescription(description, item, true);
        (void) snprintf(msg, 160, "You can't carry %s", description);
        printMessage(msg);
    }
}
