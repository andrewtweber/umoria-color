...
void playerDiedFromString(vtype_t *description, const char *monster_name, uint32_t move) {
    if ((move & config::monsters::move::CM_WIN) != 0u) {
        (void) snprintf(*description, 160, "The %s", monster_name);
    } else if (isVowel(monster_name[0])) {
        (void) snprintf(*description, 160, "an %s", monster_name);
    } else {
        (void) snprintf(*description, 160, "a %s", monster_name);
    }
}
...
void playerTakeOff(int item_id, int pack_position_id) {
    ...
    if (pack_position_id >= 0) {
        (void) snprintf(msg, 80, "%s%s (%c)", p, description, 'a' + pack_position_id);
    } else {
        (void) snprintf(msg, 80, "%s%s", p, description);
    }
    printMessage(msg);
    ...
    for (spot.y = coord.y - 1; spot.y <= coord.y + 1; spot.y++) {
        for (spot.x = coord.x - 1; spot.x <= coord.x + 1; spot.x++) {
            ...
            if (item.category_id == TV_INVIS_TRAP) {
                ...
                obj_desc_t msg = {'\0'};
                (void) snprintf(msg, 80, "You have found %s", description);
                printMessage(msg);
...
void playerGainSpells() {
    ...
    if (new_spells == 0) {
        vtype_t tmp_str = {'\0'};
        (void) snprintf(tmp_str, 80, "You can't learn any new %ss!", (stat == PlayerAttr::A_INT ? "spell" : "prayer"));
    ...
    if (new_spells == 0) {
        ...
        while (new_spells != 0) {
            ...
            vtype_t tmp_str = {'\0'};
            (void) snprintf(tmp_str, 80, "You have learned the prayer of %s.", spell_names[spell_bank[id] + offset]);
            printMessage(tmp_str);
    ...
    for (int i = blows; i > 0; i--) {
        ...
        if (py.flags.confuse_monster) {
            ...
            if (((creature.defenses & config::monsters::defense::CD_NO_SLEEP) != 0) || randomNumber(MON_MAX_LEVELS) < creature.level) {
                (void) snprintf(msg, 80, "%s is unaffected.", name);
            } else {
                (void) snprintf(msg, 80, "%s appears confused.", name);
    ...
    for (int i = 31; mask != 0u; mask >>= 1, i--) {
        if ((mask & py.flags.spells_learnt) != 0u) {
            if (msp_ptr[i].level_required > py.misc.level) {
                ...
                vtype_t msg = {'\0'};
                (void) snprintf(msg, 80, "You have forgotten the %s of %s.", p, spell_names[i + offset]);
                printMessage(msg);
            } else {
                break;
            }
    ...
    for (int n = 0; ((py.flags.spells_forgotten != 0u) && (new_spells != 0) && (n < allowed_spells) && (n < 32)); n++) {
        ...
        if ((mask & py.flags.spells_forgotten) != 0u) {
            if (msp_ptr[order_id].level_required <= py.misc.level) {
                ...
                vtype_t msg = {'\0'};
                (void) snprintf(msg, 80, "You have remembered the %s of %s.", p, spell_names[order_id + offset]);
                printMessage(msg);
            } else {
                allowed_spells++;
            }
...
static void forgetSpells(int new_spells, const char *p, int offset) {
    ...
    for (int i = 31; (new_spells != 0) && (py.flags.spells_learnt != 0u); i--) {
        ...
        if ((mask & py.flags.spells_learnt) != 0u) {
            ...
            vtype_t msg = {'\0'};
            (void) snprintf(msg, 80, "You have forgotten the %s of %s.", p, spell_names[order_id + offset]);
            printMessage(msg);
        }
    }
}
...
void playerCalculateAllowedSpellsCount(int stat) {
    ...
    if (new_spells != py.flags.new_spells_to_learn) {
        if (new_spells > 0 && py.flags.new_spells_to_learn == 0) {
            vtype_t msg = {'\0'};
            (void) snprintf(msg, 80, "You can learn some new %ss now.", magic_type_str);
            printMessage(msg);
        }
