...
static bool spellGetId(int *spell_ids, int number_of_choices, int &spell_id, int &spell_chance, const char *prompt, int first_spell) {
    ...
    vtype_t str = {'\0'};
    (void) snprintf(str, 80, "(Spells %c-%c, *=List, <ESCAPE>=exit) %s", spell_ids[0] + 'a' - first_spell, spell_ids[number_of_choices - 1] + 'a' - first_spell, prompt);
    ...
    while (!spell_found && getMenuItemId(str, spellChoice)) {
        if (isupper((int) spellChoice) != 0) {
            ...
            if (test_spell_id == number_of_choices) {
                ...
                vtype_t tmp_str = {'\0'};
                (void) snprintf(tmp_str, 80, "Cast %s (%d mana, %d%% fail)?", spell_names[spell_id + offset], spell.mana_required, spellChanceOfSuccess(spell_id));
        ...
        if (spell_id == -2) {
            vtype_t tmp_str = {'\0'};
            (void) snprintf(tmp_str, 80, "You don't know that %s.", (offset == config::spells::NAME_OFFSET_SPELLS ? "spell" : "prayer"));
...
bool spellIdentifyItem() {
    ...
    if (item_id >= PlayerEquipment::Wield) {
        ...
        (void) snprintf(msg, 160, "%s: %s", playerItemWearingDescription(item_id), description);
    } else {
        (void) snprintf(msg, 160, "%c %s", item_id + 97, description);
    }
    printMessage(msg);
    ...
    while (!finished) {
        ...
        if (tile.feature_id >= MIN_CAVE_WALL && tile.feature_id != TILE_BOUNDARY_WALL) {
            ...
            if (coordInsidePanel(coord) && caveTileVisible(coord)) {
                ...
                (void) snprintf(out_val, 160, "The %s turns into mud.", description);
