...
static void memoryConflictHistory(uint16_t deaths, uint16_t kills) {
    ...
    if (deaths != 0u) {
        (void) snprintf(desc, 80, "%d of the contributors to your monster memory %s", deaths, plural(deaths, "has", "have"));
        memoryPrint(desc);
        memoryPrint(" been killed by this creature, and ");
        if (kills == 0) {
            memoryPrint("it is not ever known to have been defeated.");
        } else {
            (void) snprintf(desc, 80, "at least %d of the beasts %s been exterminated.", kills, plural(kills, "has", "have"));
            memoryPrint(desc);
        }
    } else if (kills != 0u) {
        (void) snprintf(desc, 80, "At least %d of these creatures %s", kills, plural(kills, "has", "have"));
        memoryPrint(desc);
...
static bool memoryDepthFoundAt(uint8_t level, uint16_t kills) {
    ...
    if (level == 0) {
        ...
        vtype_t desc = {'\0'};
        (void) snprintf(desc, 80, " It is normally found at depths of %d feet", level * 50);
        memoryPrint(desc);
    }
...
static void memoryKillPoints(uint16_t creature_defense, uint16_t monster_exp, uint8_t level) {
    ...
    vtype_t desc = {'\0'};
    (void) snprintf(desc, 80, " creature is worth %d.%02d point%c", quotient, remainder, plural);
    memoryPrint(desc);
    ...
    (void) snprintf(desc, 80, " for a%s %d%s level character.", q, py.misc.level, p);
    memoryPrint(desc);
}
...
static void memoryMagicSkills(uint32_t memory_spell_flags, uint32_t monster_spell_flags, uint32_t creature_spell_flags) {
    ...
    if ((memory_spell_flags & (config::monsters::spells::CS_BREATHE | config::monsters::spells::CS_SPELLS)) != 0u) {
        ...
        if ((monster_spell_flags & config::monsters::spells::CS_FREQ) > 5) {
            vtype_t temp = {'\0'};
            (void) snprintf(temp, 80, "; 1 time in %d", creature_spell_flags & config::monsters::spells::CS_FREQ);
            memoryPrint(temp);
        }
        memoryPrint(".");
    }
...
static void memoryKillDifficulty(Creature_t const &creature, uint32_t monster_kills) {
    ...
    (void) snprintf(description, 80, " It has an armor rating of %d", creature.ac);
...
static void memoryAwareness(Creature_t const &creature, Recall_t const &memory) {
    if (memory.wake * memory.wake > creature.sleep_counter || memory.ignore == UCHAR_MAX || (creature.sleep_counter == 0 && memory.kills >= 10)) {
        ...
        vtype_t text = {'\0'};
        (void) snprintf(text, 80, " intruders, which it may notice from %d feet.", 10 * creature.area_affect_radius);
        memoryPrint(text);
    }
}
...
static void memoryLootCarried(uint32_t creature_move, uint32_t memory_move) {
    ...
    if (carrying_chance == 1) {
        ...
        vtype_t msg = {'\0'};
        (void) snprintf(msg, 80, " up to %d", carrying_chance);
        memoryPrint(msg);
    }
        ...
        if (attack_type != 1 || (dice.dice > 0 && dice.sides > 0)) {
            ...
            if ((dice.dice != 0) && (dice.sides != 0)) {
                if (knowdamage(creature.level, memory.attacks[i], dice.dice * dice.sides)) {
                    ...
                    vtype_t msg = {'\0'};
                    (void) snprintf(msg, 80, " %dd%d", dice.dice, dice.sides);
                    memoryPrint(msg);
                }
            }
        }
